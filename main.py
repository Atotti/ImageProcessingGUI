    ...
    def apply_filter(self):
        ...
        if self.mode.get() == "whole":
            if selected_filter == "bilateral":
                self.cv_image = bilateral(
                    self.cv_image,
                    d=self.bilateral_d.get(),
                ...
                self.cv_image = low_pass_filter(
                    self.cv_image, cutoff_frequency=self.lowpass_cutoff.get()
                )
            elif selected_filter == "highpass":
                self.cv_image = high_pass_filter(
                    self.cv_image, cutoff_frequency=self.highpass_cutoff.get()
                )
            ...
            scaled_points = [
                (
                    int(x * self.cv_image.shape[1] / self.image.width),
                    int(y * self.cv_image.shape[0] / self.image.height),
                )
            ...
            if selected_filter == "bilateral":
                ...
                filtered_image = low_pass_filter(
                    filtered_image, cutoff_frequency=self.lowpass_cutoff.get()
                )
            elif selected_filter == "highpass":
                filtered_image = high_pass_filter(
                    filtered_image, cutoff_frequency=self.highpass_cutoff.get()
                )
        ...
        self.image.thumbnail(
            (
                self.root.winfo_screenwidth() - self.control_frame.winfo_reqwidth(),
    ...
    def update_filter_parameters(self, *args):
        ...
        if selected_filter == "bilateral":
            ...
            Scale(
                ...
                to_=150,
                orient=HORIZONTAL,
                variable=self.bilateral_sigmaColor,
            ).pack()
            ...
            Scale(
                ...
                to_=150,
                orient=HORIZONTAL,
                variable=self.bilateral_sigmaSpace,
            ).pack()
